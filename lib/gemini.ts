...
Section shape guide:
- "list" — for "enumerated" chapters and for any explicitly named/numbered list that appears as a distinct block inside a "mixed" chapter. If the chapter contains a numbered or explicitly named list, you MUST include every single item from the source. The item count you return must match the source exactly — never compress to a representative subset, never skip items, never merge two items into one. Keep the author's original short label for each item. Each explanation is 3-5 sentences (a short paragraph) — what it means, why it matters, and enough context that someone who hasn't read the book actually understands it, not just recognizes a label. Weave in a brief example inside the explanation if one is directly tied to that specific item — don't duplicate it elsewhere.
- "steps" — for "instructional" chapters and for step-by-step blocks inside a "mixed" chapter. Capture the actual mechanism or formula in each step, not just that a step exists. Same depth as "list": 3-5 sentences per step.
- "prose" — for "argument" or "narrative" chapters, and for the continuous-prose portions of a "mixed" chapter. Generate one prose section per distinct sub-heading or thematic block in the source — if the source has 5 named sub-sections, produce 5 prose sections, not one. Never merge two different sub-topics into a single prose section; doing so silently erases content. Each prose section covers its sub-topic fully in 2-3 paragraphs. For narrative, trace the actual arc with enough detail to follow the story. For argument, walk through the reasoning step by step, not just the conclusion.
- "mixed" chapters require multiple sections of different kinds — one for each distinct content block. Example: a chapter whose first half makes a sustained argument and whose second half walks through an explicit list of chapters, techniques, or features needs BOTH a "prose" section for the argument AND a "list" section for the enumerated items. Never collapse a "mixed" chapter into a single section type — doing so silently drops half the chapter's content.
...
- "sections": generate one section per distinct sub-heading or thematic block in the source — never skip one or merge two different sub-topics together, because every merge is lost content. For enumerated chapters, one "list" section per distinct named/numbered list (never split one list across multiple sections). For argument or narrative chapters with multiple sub-headings, one "prose" section per sub-heading. A "mixed" chapter needs at least two sections of different kinds, one per distinct content block.
...
- Completeness check: before finalising your output, scan back through the chapter text and verify that every named sub-heading, distinct concept, technique, framework item, or structural section that appears in the source is represented somewhere in your sections. If anything is missing, add a section for it — never drop it to keep the output shorter.
