...
export function BookCard({ book }: BookCardProps) {
  return (
    <Link href={routes.book(book.slug)} className="group block min-w-[220px]">
      <GlassCard className="h-full overflow-hidden transition duration-300 group-hover:scale-[1.02] group-hover:border-primary/40 group-hover:shadow-glow">
        <BookCover title={book.title} author={book.author} className="rounded-b-none border-0" />
        <div className="space-y-3 p-4">
          <div>
            <h3 className="break-words font-display text-2xl font-bold leading-tight text-on-background">{book.title}</h3>
            <p className="mt-1 break-words text-sm text-on-surface-variant">{book.author}</p>
          </div>
