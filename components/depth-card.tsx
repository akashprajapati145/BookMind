  ...
  return (
    <Link href={routes.learn(slug, mode.slug)} className="group block">
      <GlassCard className="h-full p-5 transition duration-300 group-hover:scale-[1.02] group-hover:border-primary/40 group-hover:shadow-glow">
        ...
        <h3 className="break-words font-display text-3xl font-bold leading-tight text-on-background">{mode.title}</h3>
        <p className="mt-4 break-words text-sm leading-6 text-on-surface-variant">{mode.summary}</p>
      </GlassCard>
    </Link>
  );
}
